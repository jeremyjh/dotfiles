...
"     syn keyword   rustKeyword     loop match mod module move mut new of priv
"     pure
"     syn match     rustKeyword     "\<pub\>"
"     syn keyword   rustKeyword     ref return static to unchecked
"     syn match     rustKeyword     "\<unsafe\>" " Allows also matching
"     unsafe::foo()
"     syn keyword   rustKeyword     use while with
"     " FIXME: Scoped impl's name is also fallen in this category
"     syn keyword   rustKeyword     mod trait class struct enum type
"     nextgroup=rustIdentifier skipwhite
"     syn keyword   rustKeyword     fn nextgroup=rustFuncName skipwhite
"
"     syn match     rustIdentifier  contains=rustIdentifierPrime
"     "\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*"
"     display contained
"     syn match     rustFuncName
"     "\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*"
"     display contained
"
"     " Reserved words
...
"
"     syn keyword   rustType        int uint float char bool u8 u16 u32 u64
"     f32
...
"     syn keyword   rustType        c_short c_ushort c_int c_uint c_long
"     c_ulong
...
"     syn keyword   rustTrait       Add Sub Mul Div Modulo Neg BitAnd BitOr
"     BitXor
...
"     syn keyword   rustConstant    EOF SEEK_SET SEEK_CUR SEEK_END _IOFBF
"     _IONBF
"     syn keyword   rustConstant    _IOLBF BUFSIZ FOPEN_MAX FILENAME_MAX
"     L_tmpnam
"     syn keyword   rustConstant    TMP_MAX O_RDONLY O_WRONLY O_RDWR O_APPEND
"     O_CREAT
"     syn keyword   rustConstant    O_EXCL O_TRUNC S_IFIFO S_IFCHR S_IFBLK
"     S_IFDIR
"     syn keyword   rustConstant    S_IFREG S_IFMT S_IEXEC S_IWRITE S_IREAD
"     S_IRWXU
"     syn keyword   rustConstant    S_IXUSR S_IWUSR S_IRUSR F_OK R_OK W_OK
"     X_OK
...
"     contains=rustAssert " foo::<T>();
"
"     syn match     rustMacro       '\w\(\w\)*!'
"     syn match     rustMacro       '#\w\(\w\)*'
"
"     syn match     rustFormat      display "%\(\d\+\$\)\=[-+'
"     #0*]*\(\d*\|\*\|\*\d\+\$\)\(\.\(\d*\|\*\|\*\d\+\$\)\)\=\([hlLjzt]\|ll\|hh\)\=\([aAbdiuoxXDOUfFeEgGcCsSpn?]\|\[\^\=.[^]]*\]\)"
"     contained
"     syn match     rustFormat      display "%%" contained
"     syn region    rustString      start=+L\="+ skip=+\\\\\|\\"+ end=+"+
"     contains=rustTodo,rustFormat
"
"     syn region    rustAttribute   start="#\[" end="\]" contains=rustString
"
"     " Number literals
"     syn match     rustNumber      display "\<[0-9][0-9_]*\>"
"     syn match     rustNumber      display
"     "\<[0-9][0-9_]*\(u\|u8\|u16\|u32\|u64\)\>"
"     syn match     rustNumber      display
"     "\<[0-9][0-9_]*\(i8\|i16\|i32\|i64\)\>"
"
"     syn match     rustHexNumber   display "\<0x[a-fA-F0-9_]\+\>"
"     syn match     rustHexNumber   display
"     "\<0x[a-fA-F0-9_]\+\(u\|u8\|u16\|u32\|u64\)\>"
"     syn match     rustHexNumber   display
"     "\<0x[a-fA-F0-9_]\+\(i8\|i16\|i32\|i64\)\>"
"     syn match     rustBinNumber   display "\<0b[01_]\+\>"
"     syn match     rustBinNumber   display
"     "\<0b[01_]\+\(u\|u8\|u16\|u32\|u64\)\>"
"     syn match     rustBinNumber   display
"     "\<0b[01_]\+\(i8\|i16\|i32\|i64\)\>"
"
"     syn match     rustFloat       display "\<[0-9][0-9_]*\(f\|f32\|f64\)\>"
"     syn match     rustFloat       display
"     "\<[0-9][0-9_]*\([eE][+-]\=[0-9_]\+\)\>"
"     syn match     rustFloat       display
"     "\<[0-9][0-9_]*\([eE][+-]\=[0-9_]\+\)\(f\|f32\|f64\)\>"
"     syn match     rustFloat       display "\<[0-9][0-9_]*\.[0-9_]\+\>"
"     syn match     rustFloat       display
"     "\<[0-9][0-9_]*\.[0-9_]\+\(f\|f32\|f64\)\>"
"     syn match     rustFloat       display
"     "\<[0-9][0-9_]*\.[0-9_]\+\%([eE][+-]\=[0-9_]\+\)\>"
"     syn match     rustFloat       display
"     "\<[0-9][0-9_]*\.[0-9_]\+\%([eE][+-]\=[0-9_]\+\)\(f\|f32\|f64\)\>"
"
"     syn match   rustCharacter
"     "'\([^'\\]\|\\\(['nrt\\\"]\|x\x\{2}\|u\x\{4}\|U\x\{8}\)\)'"
"
"     syn region    rustComment     start="/\*" end="\*/"
"     contains=rustComment,rustTodo
"     syn region    rustComment     start="//" skip="\\$" end="$"
"     contains=rustTodo keepend
"
